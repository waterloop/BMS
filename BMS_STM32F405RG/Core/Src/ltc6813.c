					  ...
					  SPI_HandleTypeDef* ch)
{
	...
	spi_write_read(cmd,4,data,(REG_LEN*total_ic), ch);
...
void spi_write_read(uint8_t tx_Data[],//array of data to be written on SPI port
					...
					SPI_HandleTypeDef* ch)
{
	HAL_SPI_Transmit (ch, tx_Data, tx_len, 1000);  // write data to register
	HAL_SPI_Receive(ch,rx_data,rx_len,1000);
}
